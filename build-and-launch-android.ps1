...
# Ensure Android SDK variables point to a usable installation
$defaultAndroidSdk = Join-Path $env:LOCALAPPDATA 'Android\Sdk'
if (Test-Path $defaultAndroidSdk) {
    if (-not $env:ANDROID_SDK_ROOT -or $env:ANDROID_SDK_ROOT -eq ':ANDROID_SDK_ROOT') {
        $env:ANDROID_SDK_ROOT = $defaultAndroidSdk
    }
    if (-not $env:ANDROID_HOME -or $env:ANDROID_HOME -eq ':ANDROID_HOME') {
...
dotnet build $projectPath -f net10.0-android
...
if ($LASTEXITCODE -eq 0) {
    ...
    $apkDir = Join-Path $PSScriptRoot 'UnoPomodoro/bin/Debug/net10.0-android'
